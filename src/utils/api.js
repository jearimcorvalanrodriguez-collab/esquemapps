...
const initMockDB = async () => {
  if (window.localStorage.getItem('mock_initialized') !== 'v2') {
    ...
    const defaultRiders = [
      {
        ...
        content: JSON.stringify({
          proyectoId: '1718000000000',
          elements: [
            { id: 'e1', type: 'drum', x: 45, y: 30, name: 'Batería', note: 'Prueba' },
            { id: 'e2', type: 'amp', x: 20, y: 40, name: 'Amp Guitarra', note: 'Mic SM57' }
          ]
        })
      }
    ];
    ...
    window.localStorage.setItem('mock_usuarios', JSON.stringify(defaultUsuarios));
    window.localStorage.setItem('mock_proyectos', JSON.stringify(defaultProyectos));
...
const handleMockRequest = async (action, payload) => {
  ...
  const saveDB = (name, data) => window.localStorage.setItem(name, JSON.stringify(data));
  ...
  switch (action) {
    case 'login': {
      ...
      const user = usuarios.find(u => u.email.trim().toLowerCase() === email.trim().toLowerCase() && u.passwordHash === passHash);
      if (user) {
        if (user.status === 'PENDING' || user.status === 'INACTIVO') {
          return { status: 'error', message: 'Cuenta bloqueada o en revisión.' };
        }
    ...
    case 'checkEmailTC': {
      const usuarios = getDB('mock_usuarios');
      const user = usuarios.find(u => u.email.trim().toLowerCase() === payload.email.trim().toLowerCase());
      return { status: 'success', accepted: user ? !!user.disclaimerAceptado : false };
    }
    ...
    case 'updateProfile': {
      const { email, name, phone, dieta, disclaimerAceptado, password } = payload;
      const usuarios = getDB('mock_usuarios');
      ...
      if (index !== -1) {
        ...
        if (disclaimerAceptado !== undefined) usuarios[index].disclaimerAceptado = disclaimerAceptado;
        if (password) {
    ...
    case 'recuperarClave': {
      return { status: 'success', message: 'Se ha simulado el envío de recuperación. Tu contraseña sigue siendo la misma.' };
    }
    ...
    case 'deleteProyecto': {
      ...
      hitos = hitos.map(h => String(h.proyectoId) === String(id) ? { ...h, proyectoId: '' } : h);
    ...
    case 'updateRider': {
      ...
      const index = riders.findIndex(r => String(r.id) === String(payload.id));
      if (index !== -1) {
        riders[index] = {
          ...riders[index],
          title: payload.title,
    ...
    default:
      return { status: 'error', message: `Acción '${action}' no soportada en el modo local.` };
  }
};
...
export const normalizeRole = (role) => {
  ...
  if (clean === 'ADMIN' || clean === 'MANAGER' || clean === 'TOUR MANAGER') return 'TOUR MANAGER';
  return 'TÉCNICO';
};
...
export const apiFetch = async (action, payload = {}) => {
  ...
  if (isMock) {
    ...
    if (!url) {
      throw new Error("No VITE_GAS_URL defined in environment and VITE_LOCAL_MOCK is false.");
    }
...
export const formatCleanLocation = (loc) => {
  ...
  const countryKeywords = [
    'chile', 'argentina', 'peru', 'bolivia', 'brasil', 'colombia', 
    'ecuador', 'uruguay', 'paraguay', 'venezuela', 'españa', 'mexico', 
    'united states', 'usa', 'united kingdom', 'uk'
  ];
  const regionKeywords = [
    'región', 'region', 'metropolitana', 'provincia', 'estado', 
    'state', 'county', 'departamento', 'rm'
  ];
